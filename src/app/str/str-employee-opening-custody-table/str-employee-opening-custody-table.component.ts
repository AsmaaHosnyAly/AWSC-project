...
import { EmployeeCustodyPrintDialogComponent } from 'src/app/str/employee-custody-print-dialog/employee-custody-print-dialog.component';
...
export class STREmployeeOpeningCustodyTableComponent implements OnInit {
  ...
  downloadPdf(no: any, date: any, itemId: any) {
    console.log('no. : ', no, 'item : ', itemId, 'date: ', date);
    ...
    this.api
      .getStrEmployeeCustody(no, costCenterId, employeeId, date, itemId)
      .subscribe({
        next: (res) => {
    ...
    this.api
      ...
      .subscribe({
        next: (res) => {
          ...
          this.pdfurl = url;
          this.dialog.open(EmployeeCustodyPrintDialogComponent, {
            width: '50%',
          });
