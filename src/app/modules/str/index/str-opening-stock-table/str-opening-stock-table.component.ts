...
interface StrOpeningStockDetails {
  itemName: any;
  price: any;
  qty: any;
  total: any;
...
export class StrOpeningStockTableComponent implements OnInit {
  ...
  // dataSourceDetails: MatTableDataSource<StrOpeningStockDetails> = new MatTableDataSource();
  ...
  dataSourceDetails: MatTableDataSource<StrOpeningStockDetails> =
    new MatTableDataSource();
  pageIndexDetails: any;
  lengthDetails: any;
  // pageSizeDetails: any;
  pageSizeDetails = 5;
  ELEMENT_DATA_DETAILS: StrOpeningStockDetails[] = [];
  ...
  storeValueChanges(storeId: any) {
    ...
    this.groupMasterFormSearch.controls['storeId'].setValue(
  ...
  //   alert('nnnnnn'+this.getMasterRowId)
  ...
  getAllDetailsForms() {
    ...
    // console.log("mastered row get all data: ", this.getMasterRowId)
    // if (this.getMasterRowId) {
    //   this.api.getStrOpenDetailsByMasterId(this.getMasterRowId.id).subscribe({
    //     next: (res) => {
    //       this.matchedIds = res;
    ...
    if (this.getMasterRowId) {
      ...
      if (!this.currentPageDetails) {
        ...
        // this.isLoading = true;
        fetch(
          this.api.getStrOpenDetailsPaginateByMasterId(
            this.currentPageDetails,
            this.pageSizeDetails,
  ...
  async nextToAddFormDetails() {
    ...
    if (
      ...
      this.api.postStrOpen(this.groupMasterForm.value).subscribe({
        next: (res) => {
          this.getMasterRowId = {
            id: res,
          };
  ...
  getAllMasterForms() {
    if (!this.currentPage && this.serachFlag == false) {
      ...
      // this.isLoading = true;
      fetch(
        this.api.getStrOpeningStockPaginate(this.currentPage, this.pageSize)
      )
        .then((response) => response.json())
        .then(
          (data) => {
  ...
  storeSelected(event: MatAutocompleteSelectedEvent): void {
    ...
    console.log(
      'store in form: ',
      this.groupMasterFormSearch.getRawValue().storeId
    );
  }
  ...
  async addDetailsInfo() {
    ...
    if (!this.editDataDetails) {
      ...
      if (this.editData) {
        this.groupDetailsForm.controls['stR_Opening_StockId'].setValue(
          this.editData.id
        );
      } else {
        this.groupDetailsForm.controls['stR_Opening_StockId'].setValue(
      ...
      if (this.groupDetailsForm.valid && !this.getDetailedRowData) {
        this.api.postStrOpenDetails(this.groupDetailsForm.value).subscribe({
          next: (res) => {
            this.toastrSuccess();
            this.groupDetailsForm.reset();
            // this.updateDetailsForm();
    ...
    if (code.keyCode == 13) {
      this.productsList.filter((a: any) => {
        ...
        if (a.code == code.target.value) {
          console.log(
            'enter product code case condition: ',
            a.code === code.target.value
          );
