...
import { PyExchangeDialogComponent } from '../py-exchange-dialog/py-exchange-dialog.component';
...
@Component({
  selector: 'app-py-exchange',
  templateUrl: './py-exchange.component.html',
  styleUrls: ['./py-exchange.component.css']
})
export class PyExchangeComponent  implements OnInit {
  ELEMENT_DATA: USER[] = [];
  isLoading = false;
  totalRows = 0;
  pageSize = 5;
...
this.getfiscalYear();
  ...
  openPyExchangeDialog() {
    this.dialog
      .open(PyExchangeDialogComponent, {
        width: '95%',
        height: '79%'
      })
      .afterClosed()
  ...
  getAllMasterForms() {
    ...
    if (!this.currentPage) {
      ...
      fetch(this.api.getPyExchangePaginate(this.currentPage, this.pageSize))
        ...
        next: (res) => {
          ...
          if (this.matchedIds.length) {
            for (let i = 0; i < this.matchedIds.length; i++) {
              ...
              if (result) {
                this.api.deletePyExchangeDetails(this.matchedIds[i].id).subscribe({
                  next: (res) => {
                    console.log('master id to be deleted: ', id);

                    this.api.deletePyExchange(id).subscribe({
                      next: (res) => {
